      ...
      {experiences.map((exp, idx) => (
        <div key={idx}>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            <div>
              <h3 className="text-xl font-semibold text-[#815634]">{exp.title}</h3>
              <div className="flex items-center gap-2">
                <span className={`text-sm px-2 py-1 rounded ${exp.companyBg} font-semibold flex items-center gap-1 min-w-[120px]`}>
            ...
            <span className="text-slate-400 text-sm md:text-right">{exp.location}</span>
          </div>
